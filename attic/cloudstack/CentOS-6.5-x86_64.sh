...
function usage
{
    echo "Script for builing CentOS 6.5 images"
    ...
    echo "timezone=GMT \\"
    echo "bash CentOS-6.5-x86_64.sh\\"
}
...
sed -i.bak -e "s/timezone\ .*/timezone\ $timezone/" http_directory/centos-6.5/ks.cfg
...
for size in "${!swap_size[@]}"; do
    ...
    sed -i.bak -e "s/logvol\ swap\ --fstype\ swap\ --name\=swap\ --vgname\=vg0\ --size\ .*/logvol\ swap\ --fstype\ swap\ --name\=swap\ --vgname\=vg0\ --size\ ${swap_size["$size"]}/" http_directory/centos-6.5/ks.cfg
    packer build \
      -only=qemu \
...
sed -i.bak -e 's/logvol\ swap\ --fstype\ swap\ --name\=swap\ --vgname\=vg0\ --size\ .*/logvol\ swap\ --fstype\ swap\ --name\=swap\ --vgname\=vg0\ --size\ 512/' http_directory/centos-6.5/ks.cfg
